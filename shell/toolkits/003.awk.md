...
- [awk](#awk)
  ...
  - [案例](#案例)
    ...
    - [收集 FS NO\_ANSWER 错误](#收集-fs-no_answer-错误)
...
interval=10
maxCounter=10
notifyURI='https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=fac2f0f1-c3bb-4976-a5ff-d24de5349cce'
notifyBodyTmpl='{"msgtype":"text","text":{"content":"CONTENT","mentioned_mobile_list":["18217562304"]}}'

lastTime=0; counter=0; tail -F /usr/local/freeswitch/log | grep --line-buffered 'Hangup sofia/internal/' | grep --line-buffered NO_ANSWER | while read line; do
    channelID=`echo $line | awk '{print $1}'`;
    datetime=`echo $line | awk '{print $2"T"$3}'`;
    timestamp=`date +%s -d "${datetime}"`;
    extNumber=`echo $line | grep -Eo 'sofia/internal/[^@ ]*'`;
    extNumber="${extNumber:15}";
    ...
    echo "${channelID} ${datetime} ${timestamp} ${extNumber} ${lag}s ${counter}/${maxCounter}" >> /var/log/kylin_cti/fs-no-answer-monitor.log;

    if [ ${counter} -ge ${maxCounter} ]; then
        replacement="`hostname` has got NO_ANSWER exceed ${maxCounter} times with interval ${interval}s continuously."
        notifyBody=${notifyBodyTmpl/"CONTENT"/$replacement}

        counter=0
        curl "${notifyURI}" -H 'Content-Type: application/json' -d "${notifyBody}" >> /var/log/kylin_cti/fs-no-answer-monitor.log;
    fi
done

# external NORMAL_CLEARING
interval=100
maxCounter=2
...
lastTime=0; counter=0; tail -F /usr/local/freeswitch/log | grep --line-buffered 'Hangup sofia/external/' | grep --line-buffered NORMAL_CLEARING | while read line; do
    channelID=`echo $line | awk '{print $1}'`;
    datetime=`echo $line | awk '{print $2"T"$3}'`;
    timestamp=`date +%s -d "${datetime}"`;
    extNumber=`echo $line | grep -Eo 'sofia/external/[^@ ]*'`;
    extNumber="${extNumber:15}";

    lag=$[timestamp - lastTime];
    echo "${channelID}, ${datetime}, ${timestamp}, ${extNumber}, ${lag}"
    ...
    echo "${channelID}, ${datetime}, ${timestamp}, ${extNumber}, ${lag}" >> /var/log/kylin_cti/fs-no-answer-monitor.log;
    echo "$counter, $maxCounter";

    if [ ${counter} -ge ${maxCounter} ]; then
        replacement="`hostname` has got NORMAL_CLEARING exceed ${maxCounter} times with interval ${interval}s continuous."
        echo $replacement;
        notifyBody=${notifyBodyTmpl/"CONTENT"/$replacement};
        echo $notifyBody;

        counter=0
        curl "${notifyURI}" -H 'Content-Type: application/json' -d "${notifyBody}"
    fi
done
