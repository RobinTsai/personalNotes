...
```sh
# 解析 RTP，查看语音编码，8 表示 8000HZ
# r: read file; d: <筛选条件>,<解析类型>; T: 输出格式; e: 当 -T fields 时使用，表明哪个 Field
tshark -nr ./1003-1.pcap -d udp.port==31576,rtp -T fields -e rtp.p_type | head -n 1
# 提取 payload，
# xxd 是 hex 格式转换，-r 是 hex -> binary，-p
tshark -nr ./1003-1.pcap -d udp.port==31576,rtp -T fields -e rtp.payload | xxd -r -p > ./1.al
```
